...
object RollbackIndexGuard {
    ...
    val MAX_VALUE: BigInteger = BigInteger.valueOf(2).pow(64).subtract(BigInteger.ONE)
    ...
    fun classify(value: BigInteger, nowSeconds: Long): RollbackIndexVerdict {
        ...
        // bitLength <= 63 guarantees the value fits in a signed Long, so the
        // string round-trip is exact (longValueExact needs API 31).
        val epoch: Long? = if (value.bitLength() <= 63) value.toString().toLong() else null
